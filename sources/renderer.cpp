...
Eigen::Vector4d Renderer::getScreenPoint(Eigen::Vector4d point) {
    return camera->transformToCube(point);
}

void Renderer::drawPoint(Eigen::Vector4d point, Uint32 pixelColor) {
    if (point[3] == 0) {
        return;
    }
    Eigen::Vector3d info = screen->getPixel(point);
    screen->drawPoint(info[0], info[1], info[2], pixelColor);
}

void Renderer::drawLine(Eigen::Vector4d point1, Eigen::Vector4d point2, Uint32 pixelColor1, Uint32 pixelColor2) {
    Eigen::Vector3d info1 = screen->getPixel(point1);
    Eigen::Vector3d info2 = screen->getPixel(point2);
    screen->drawPixelLine(info1, info2, pixelColor1, pixelColor2);
}

void Renderer::drawTriangle(Eigen::Vector4d point1, Eigen::Vector4d point2, Eigen::Vector4d point3, Uint32 pixelColor1, Uint32 pixelColor2, Uint32 pixelColor3) {
    Eigen::Vector3d light = camera->getLightVector();
    Eigen::Vector3d v1 = point2.head(3) - point1.head(3);
    Eigen::Vector3d v2 = point3.head(3) - point1.head(3);
    Eigen::Vector3d norm = v2.cross(v1);
    norm.normalize();
    ...
    if (point1[2] >= 0) {
        ...
        point2 = point1 + (point2 - point1) * (-point1[2]) / (point2[2] - point1[2]) + Eigen::Vector4d(0, 0, -0.2, 0);
        point3 = point1 + (point3 - point1) * (-point1[2]) / (point3[2] - point1[2]) + Eigen::Vector4d(0, 0, -0.2, 0);
    } else if (point2[2] < 0 && point3[2] >= 0) {
        Eigen::Vector4d point4 = point1 + (point3 - point1) * (-point1[2]) / (point3[2] - point1[2]) + Eigen::Vector4d(0, 0, -0.2, 0);
        //Uint32 pixelColor4 = screen->mixColors(pixelColor1, pixelColor3, (-point1[2]) / (point3[2] - point1[2]));
        Eigen::Vector4d point5 = point2 + (point3 - point2) * (-point2[2]) / (point3[2] - point2[2]) + Eigen::Vector4d(0, 0, -0.2, 0);
    ...
    Eigen::Vector3d info1 = screen->getPixel(point1);
    Eigen::Vector3d info2 = screen->getPixel(point2);
    Eigen::Vector3d info3 = screen->getPixel(point3);
...
void Renderer::drawEdge(Object &obj, std::pair<int, int> edgeBorders) {
    Eigen::Vector4d shift = obj.getPosition();
...
void Renderer::addFile(const std::string& s, Eigen::Vector4d pos) {
    ObjectParser parser;
...
void Renderer::moveCamera(Eigen::Vector4d move) {
    camera->move(move);
}
