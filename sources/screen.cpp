...
Vector3d Screen::getPixel(Vector4d point) {
    ...
    result[0] = floor((result[0] + 1) / pixelWidth);
    result[1] = floor((result[1] + 1) / pixelHeight);
    return result;
}
...
void Screen::drawPixelLine(Vector3d point1, Vector3d point2, Uint32 pixelColor1, Uint32 pixelColor2) {
    ...
    for (int x = point1[0]; x <= point2[0]; ++x) {
        double t = (x - point1[0]) / (double) (point2[0] - point1[0]);
        int y = point1[1] * (1.0 - t) + point2[1] * t;
        double z = point1[2] * (1.0 - t) + point2[2] * t;
        Uint32 color = pixelColor1 * (1.0 - t) + pixelColor2 * t;
        std::cout << "HERE " << x << ' ' << y << ' ' << z << '\n';
        if (steep) {
...
void Screen::drawTriangle(Vector3d point1, Vector3d point2, Vector3d point3, Uint32 color1, Uint32 color2, Uint32 color3) {
    ...
    if (lower_height == 0) {
        ...
        for (int i = point1[1]; i <= point2[1]; ++i) {
            ...
            Uint32 colorA = color1 * (1 - k3) + color3 * k3;
            Uint32 colorB = color1 * (1 - k2) + color2 * k2;
            if (A[0] > B[0]) {
                std::swap(A, B);
            ...
            for (int j = A[0]; j <= B[0]; ++j) {
                ...
                Uint32 color = colorA * (1 - k) + colorB * k;
    ...
    if (upper_height == 0) {
        ...
        for (int i = point2[1]; i <= point3[1]; ++i) {
            ...
            for (int j = A[0]; j <= B[0]; ++j) {
                double k = (j - A[0]) / std::max(1.0, B[0] - A[0]);
                Uint32 color = colorA * (1 - k) + colorB * k;
                drawPoint(j, i, A[2] * (1 - k) + B[2] * k, /*color*/ 255 << 8);
            }
        }
    }
